...
//삭제 요청 처리  ( DB삭제 + 이미지삭제 ) 
//파일을 업로드 하건, 안 하건 일단 multer를 사용하게 되면, 모든 post방식에 관려하게 되어있다.
...
//수정요청 처리
//일단 업로드 컴포넌트 사용하게 되면, post는 무조건 업로드 컴포넌트 이용해야한다.
app.post("/gallery/edit", upload.single("pic"), function(request, response){
    ...
    //클라이언트가 업로드를 원하는지 안 하는지 구분
    //console.log("request.file", request.file);
    //업로드 시, request 객체의 json 속성 중 file이라는 속성이 판단 대상.
